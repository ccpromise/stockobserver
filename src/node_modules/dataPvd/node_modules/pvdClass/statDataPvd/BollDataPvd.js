...
BollDataPvd.prototype._calculate = function(ts) {
    ...
    var n = this._N;
    while(n > 0) {
        data.push(this._pvd.get(ts));
        ts = this._pvd.backwardDateTs(ts, 1);
        n--;
    }
...
function checkParams(paramObj) {
    return validate.isObj(paramObj) && (object.numOfKeys(paramObj) === 2 || object.numOfKeys(paramObj) === 3)
    && validate.isPosInt(paramObj.N) && (paramObj.K === undefined || validate.isPosInt(paramObj.K))
    && pvdGenerator.checkldp(paramObj.pvd);
}

function pvdID(paramObj) {
    return 'boll' + '_' + paramObj.N + '_' + (paramObj.K || config.bollingerK)+ '_' + pvdGenerator.pvdID(paramObj.pvd);
}

/**
 * referenceID: key - this.id
 *              value - this._pvd.id (the id of its dependency)
 */
...
function makePvd(paramObj, id) {
    ...
    return pvdGenerator.makePvd(paramObj.pvd).then((pvd) => {
        return new BollDataPvd(pvd, paramObj.N, paramObj.K || config.bollingerK, id);
    });
}
