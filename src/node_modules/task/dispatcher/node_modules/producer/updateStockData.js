...
const getSecID = require('datasrc').getSecID;
const tradeplans = require('tradePlans').list;
...
function insertSimulateTask(list, objIdMap) {
    var allDocs = [];
    tradeplans.forEach((tradeplanId) => {
        var docs = list.map((secID) => {
            return {
                task: {
                    type: 'simulate',
                    pack: { tradeplanId: tradeplanId, secID: secID }
                },
                status: taskStatus.wait,
                readyCondition: {
                    type: 'or',
    ...
    return taskCol.insertMany(allDocs);
}
