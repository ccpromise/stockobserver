...
function Cache(opt) {
    opt = opt || {};
    opt.maxCacheSize = opt.maxCacheSize || config.maxCacheSize;
    opt.maxCacheTime = opt.maxCacheTime || config.maxCacheTime;
    if(opt.maxCacheSize <= 0 || opt.maxCacheTime <= 0) throw new Error('cache size and time should be positive');
    this._maxSize = opt.maxCacheSize;
    this._cache = Object.create(null);
    this._referTo = Object.create(null);
    this._createTime = Object.create(null);
    this._referredBy = Object.create(null);
    this._list = new LinkedList();
    this._deleteRef = opt.deleteRef || false;
    this._maxCacheTime = opt.maxCacheTime;
    this.clearTimeout();
}

Cache.prototype.clearTimeout = function () {
    var that = this;
    var loop = function () {
        setTimeout(() => {
            var now = time.getTs(time.now());
            Object.keys(that._createTime).forEach((key) => {
                if(now - that._createTime[key] > that._maxCacheTime) {
                    that._delete(key);
                }
            });
        }, that._maxCacheTime / 100);
    }
    loop();
}
...
Cache.prototype.set = function(key, val) {
    ...
    if(!(key in this._cache)) this._createTime[key] = time.getTs(time.now());
    if(key in this._referredBy) {
        for(let ref of this._referredBy[key]) {
            this.set(ref, this._cache[ref]);
        }
