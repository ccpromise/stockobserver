...
module.exports = function (planIds) {
    return getStockList().then((list) => {
        var i = 0, N = planIds.length;

        return async.while(() => {
            return i < N;
        }, () => {
            //* simulation for i-th trade plans
            var id = planIds[i++];
            ...
            return async.while(() => {
                ...
                var secIDs = list.slice(j, j + numberOfSecIDPerReq);
                j += numberOfSecIDPerReq;
                //* get open trades from database
                return httpReq('/simulate', { filter: { tradeplanId: id, secID: { $in: secIDs }, closed: false } }, 'find').then((trades) => {
                    var tradeMap = new Map(secIDs.map((id) => [id, []])), oldTrade = [], newTrade = [];
